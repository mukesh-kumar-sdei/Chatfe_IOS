...
//    static let shared = HSAppleSignIn()
    ...
    @available(iOS 13.0, *)
    func loginWithApple(button: UIButton,type:ASAuthorizationAppleIDButton.ButtonType? = .signIn,style:ASAuthorizationAppleIDButton.Style? = .white,completionBlock:AppleSignInBlock){
        button.addTarget(self, action: #selector(self.didTapLoginWithApple), for: .touchUpInside)
